...
/**
 * Simulates competing bacteria colonies using a rock-paper-scissors model. Each cell in the grid
 * represents a bacteria colony that can be in one of three states: ROCK, PAPER, or SCISSORS. The
 * simulation follows rock-paper-scissors rules where:
 * <ul>
...
public class BacteriaColoniesSimulation extends Simulation {
  ...
  /**
   * Constructs a new BacteriaColoniesSimulation with specified configuration and grid. Initializes
   * the simulation grid with the default state and sets up the neighbor threshold for state
   * changes. The neighbor threshold determines how many neighbors of a competing state are needed
   * to change a cell's current state.
   *
   * @param simulationConfig Configuration containing grid dimensions, initial states, and
   *                         parameters including the neighbor threshold
  ...
  /**
   * Initializes the mapping between states and their display colors. ROCK is displayed as red,
   * PAPER as green, and SCISSORS as blue.
   *
  ...
  /**
   * Applies the rock-paper-scissors rules to all cells in the grid. For each cell, counts its
   * neighbors' states and determines if the cell should change state based on the neighbor
   * threshold.
   */
  @Override
  ...
  /**
   * Updates the state of a single cell based on its Moore neighbors. A cell changes its state to a
   * competing state if the number of neighbors in that state exceeds the neighbor threshold. The
   * competing state must be one that beats the current state in rock-paper-scissors rules.
   *
  ...
  /**
   * Counts the number of neighbors in each possible state. Creates a mapping from each bacteria
   * state to the number of neighboring cells currently in that state.
   *
